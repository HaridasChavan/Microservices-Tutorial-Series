...
public class UserServiceImpl implements UserService {
	...
	public User getUser(String userId) {
		...
		User user = userRepositoy.findById(userId).orElseThrow(
				() -> new ResourceNotFoundException("User with given id is not found on server:!!" + userId));

		// fetch rating of the above user RATING SERVICE
		// http://localhost:8084/ratings/users/00dcdde5-5a2f-4b32-ba37-591c96c900b4

		/*
		 * ArrayList<Rating> ratingsOfUser = restTemplate
		 * .getForObject("http://localhost:8084/ratings/users/" + user.getUserId(),
		...
		List<Rating> ratingList = ratings.stream().map(rating -> {
			...
			// http://localhost:8083/hotels/672a3291-986c-4b36-a261-379a920e4c7c
